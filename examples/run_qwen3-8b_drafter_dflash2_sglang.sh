...
# GPU example for a DFlash2 drafter co-trained against SGLang's DFLASH worker.
#
# DFlash2 (dynamic convolutions + candidate selector) is served by sglang main
# through the DFLASH speculative worker; the engine picks the DFlash2 draft
# from the checkpoint's "DFlash2DraftModel" architecture and reads the module
# hyperparameters from dflash_config (z-lab layout; convert a speculators-format
# drafter with `python -m verl_speco.convert_speculators_dflash2`).
...
DRAFTER_PATH=/path/to/sglang-compatible-dflash2-drafter
...
PYTHONUNBUFFERED=1 python3 -m verl_speco.main \
    ...
    actor_rollout_ref.rollout.drafter.rollout.spec_verify_tokens=8 \
